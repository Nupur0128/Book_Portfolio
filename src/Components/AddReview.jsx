...
function AddReview({ isOpen, onClose, onReviewAdded }) {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50" onClick={onClose}>
            <div className="max-w-2xl w-full mx-4 p-6 bg-[#1D3854] rounded-lg shadow-xl" onClick={(e) => e.stopPropagation()}>
                <h2 className="text-2xl font-bold mb-6 text-white">Add Your Review</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex gap-4">
                        <button type="button" onClick={onClose} className="w-full bg-red-600 text-white py-2 px-4 rounded-md hover:bg-red-800 transition-colors font-medium">Cancel</button>
