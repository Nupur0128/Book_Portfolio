...
function BuyNow() {
  ...
  return (
    <div className='relative px-4 py-8 w-full overflow-hidden' style={{ background: 'linear-gradient(to bottom, #312e81, #1e1b4b, #0f172a)' }}>
      ...
      <h1 className="text-5xl pb-2 text-white font-bold text-center drop-shadow-lg z-10">Choose Your Plan</h1>
      ...
      <div className='text-center pb-6 z-10'>
        <div className='inline-flex bg-white/10 backdrop-blur-md rounded-full p-1 shadow-lg border border-white/20'>
          ...
          <button 
            className={`px-6 py-2 rounded-full transition-all duration-300 ${activeTab === 'yearly' ? 'bg-gradient-to-r from-blue-500 to-purple-500 text-white shadow-lg' : 'text-gray-200 hover:text-white'}`}
      ...
      <div className='container mx-auto gap-8 items-center grid grid-cols-1 md:grid-cols-4'>
        {
          (activeTab === 'monthly' ? monthly : yearly).map((plan, index) => (
            ...
            >
              ...
              <button className='w-full py-3 px-4 rounded-xl bg-gradient-to-r from-blue-500 to-purple-500 text-white font-semibold hover:from-blue-600 hover:to-purple-600 transition shadow-md hover:shadow-xl hover:-translate-y-1'>
                Get Started
              </button>
            </div>
          ))
